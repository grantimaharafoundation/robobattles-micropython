...
static pbdrv_bluetooth_ad_match_result_flags_t xbox_advertisement_response_matches(uint8_t event_type, const uint8_t *data, const char *name, const uint8_t *addr, const uint8_t *match_addr) {
    ...
    if (memcmp(addr, match_addr, 6) == 0) {
        flags |= PBDRV_BLUETOOTH_AD_MATCH_ADDRESS;
        // Not necessary to remember address here, since it will have been remembered earlier
    }
...
static mp_obj_t pb_type_xbox_make_new(const mp_obj_type_t *type, size_t n_args, size_t n_kw, const mp_obj_t *args) {
    ...
    // Unfortunately have to force disconnect with Prime Hub too for a bluetooth freeze workaround. Can be reproduced by entering
    // bluetooth mode, connecting hub to computer, pressing play to load program with Xbox controller support
    // onto hub, connecting controller, turning off hub, then repeating steps. Eventually when pressing the bluetooth button on the IDE, the
    // button wheel will appear forever, forcing a browser refresh. After refreshing and repeating the process, bt will connect but after loading
    // the program and connecting the controller, this error will appear "Traceback (most recent call last): File "my_file.py", line 5, in <module>
    // RuntimeError: Unknown error" where line number is the line with XboxController(). The issue will persist until you turn the hub off and on,
    // and (if not using autostart) click the bt button in the IDE to disconnect the hub from the computer.
    #if PYBRICKS_HUB_TECHNICHUB || PYBRICKS_HUB_PRIMEHUB
    if (!mp_obj_is_true(stay_connected_in) && pbdrv_bluetooth_is_connected(PBDRV_BLUETOOTH_CONNECTION_PYBRICKS)) {
        ...
        mp_printf(&mp_plat_print, "The hub has disconnected from the computer so it can be used with the Xbox controller.\n");
        mp_hal_delay_ms(500);
    }
    #endif // PYBRICKS_HUB_TECHNICHUB || PYBRICKS_HUB_PRIMEHUB
