...
static uint32_t default_user_program_light_animation_next(pbio_light_animation_t *animation) {
    // Check for active Xbox controller connection
    ...
    if (!xbox_connected) {
        ...
        uint8_t current_tick = animation_progress;
        animation_progress = (animation_progress + 1) % total_ticks;

        if (current_tick < fade_up_ticks) {
            // Fade to full white: 0 to 7
            hsv.v = (current_tick * 100) / fade_up_ticks;
        } else if (current_tick < fade_up_ticks + on_ticks) {
            // Stay fully lit up white: 8 to 36
            hsv.v = 100;
        } else if (current_tick < fade_up_ticks + on_ticks + fade_down_ticks) {
            // Fade to off: 37 to 44
            uint8_t elapsed = current_tick - (fade_up_ticks + on_ticks);
